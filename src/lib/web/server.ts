...
import { handleReact, handleCorrect, handleNote, handleDone } from "./handlers.js";
...
export async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    ...
    if (method === "POST" && url.pathname === "/note") return await handleNote(req, res);
    if (method === "POST" && url.pathname === "/done") return await handleDone(req, res);
